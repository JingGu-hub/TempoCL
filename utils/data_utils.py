...
def build_dataset(args):
    ...
    if args.label_noise_rate > 0 or args.ood_noise_rate > 0:
        train_dataset, train_noisy_target, mask_train_target = flip_label(args=args, dataset=train_dataset, target=train_target, ratio=args.label_noise_rate)

    # load train_loader
    train_loader = load_loader(args, train_dataset, train_target, noisy_target=train_noisy_target, mode='train', ood_ids=ood_ids)
    # load test_loader
    test_loader = load_loader(args, test_dataset, test_target, shuffle=False, mode='test', ood_ids=ood_ids)

    return train_loader, test_loader, train_dataset, train_target, train_noisy_target, test_dataset, test_target, input_channel, seq_len, num_classes, ood_ids

class TimeDataset(data.Dataset):
    def __init__(self, dataset, target=None, noisy_label=None, pred=None, prob=None, ood_mask=None, ood_ids=None, mode='train'):
...
def load_loader(args, data, target, noisy_target=None, pred=None, prob=None, shuffle=True, mode='train', ood_mask=None, ood_ids=None):
    if mode == 'train':
